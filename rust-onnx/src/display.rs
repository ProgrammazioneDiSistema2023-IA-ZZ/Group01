...
use crate::utils::shared::{Model, MODELS_NAMES};
...
fn print_intro() {
 ...
 ----------------------------------------------------------------------------------------------------------------------------------------------
    ...
    - 📦 Delivered with a set of validated operators, neural networks and inputs.
    - ➕  Easy to add new operators to extend model compatibility.
    - 🖼️Dataset (images & labels) serialization to extend the set of available inputs.
...
pub fn menu() -> (Model, bool, bool, String){
    print_intro();
        ...
        .unwrap()
    {
        ...
        2 => {
            if let Err(e) = clear_screen() {
                eprintln!("Error clearing the screen: {}", e);
            }
            return menu();
        }
        _ => false,
                        ...
                        if !dir_name.parse::<i32>().is_err(){
                            ...
                            for sub_entry in fs::read_dir(path).unwrap(){
                                ...
                                match sub_path.extension().and_then(|ext| ext.to_str()) {
                                    Some("jpg") | Some("png") | Some("jpeg") | Some("JPEG") | Some("PNG") | Some("JPG")=> jpg_jpeg_png_flag =true,
                                    _ => {},
                                }
                            }
    ...
    println!();
    (chosen_model, verbose, test_dataset, folder_name)
