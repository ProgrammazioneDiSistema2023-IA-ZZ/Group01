...
pub fn serialize_images (custom_dataset_path: &PathBuf, custom_dataset_serialized_path: &PathBuf, chosen_model: &Model) ->Result<(),Box<dyn std::error::Error>>{
                    ...
                    Ok(label_dir) => {
                        ...
                        for file_res in files_dir {
                            ...
                            match file.path().extension().and_then(|ext| ext.to_str()) {
                                Some("jpg") | Some("png") | Some("jpeg")| Some("JPEG") | Some("PNG") | Some("JPG")=> {},
                                _ => continue,
                            }
                            ...
                            match chosen_model {
                                Model::MNIST => {
                                    ...
                                    num_classes = chosen_model.get_num_classes();
                                },
                                Model::ResNet18v17 | Model::MobileNetv27 | Model::ResNet34v27 => {
