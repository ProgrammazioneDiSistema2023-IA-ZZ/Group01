...
impl Conv {
    ...
    fn execute_conv_optimized(x: ArrayD<f32>, w: ArrayD<f32>, b: Option<&ArrayD<f32>>, auto_pad: &Option<AutoPad>, dilations: &Vec<usize>, kernel_shape: &Vec<usize>, pads: &Vec<usize>, strides: &Vec<usize>) -> Result<Vec<ArrayD<f32>>, String> {
        ...
        let kernel_height = w_copy.shape()[2];
        let kernel_width = w_copy.shape()[3];
        let output_height = (x.shape()[2] + 2 * pads_copy[0] - kernel_height) / strides[0] + 1;
        let output_width = (x.shape()[3] + 2 * pads_copy[0] - kernel_width) / strides[0] + 1;

        //STEP 2
        let out_channels = w_copy.shape()[0];
        let flattened_size = w_copy.len() / out_channels;
        let binding = w_copy.clone().into_shape((out_channels, flattened_size)).unwrap();
        let w_copy_2d = binding.t();

        //STEP 3 - result will be 64x12544
        let dot_result = im2col_res.dot(&w_copy_2d);
    ...
    fn im2col_fast(x: ArrayBase<OwnedRepr<f32>, IxDyn>, kernel_shape: &Vec<usize>, pads: &Vec<usize>, strides: &Vec<usize>) -> ArrayBase<OwnedRepr<f32>, Ix2> {//-> Vec<usize>{
        ...
        let output_height = (height + 2 * pad - kernel_height) / stride + 1;
        let output_width = (width + 2 * pad - kernel_width) / stride + 1;
